...
RSpec.describe '記録投稿', type: :system do
  ...
  context '記録投稿ができないとき' do
    it '入力途中でページを移動した時' do
      ...
      # 動画を投稿し記録ページへ遷移する
      click_button '比較する'
      # topに移動するとカウントが上がらないことを確認する
      expect { visit root_path }.to change { Compare.count }.by(0)
    end
    it 'ログインしていないと新規投稿ページに遷移できない' do
      # トップページに遷移する
      visit root_path
...
RSpec.describe '編集機能', type: :system do
  ...
  context '編集ができない時' do
    ...
    it 'ログインしていないとユーザーの詳細ページにいけない' do
      ...
      # skill編集ページへ行こうとするとログインページに遷移する
      visit edit_skill_path(@skill1)
      expect(current_path).to eq new_user_session_path
      # compare編集ページへ行こうとするとログインページに遷移する
      visit edit_skill_compare_path(@compare.skill, @compare)
      expect(current_path).to eq new_user_session_path
    end
    ...
    it 'ログインしているが誤った情報ではそのページにとどまる' do
      ...
      fill_in 'notice', with: ''
      # ページにとどまる
      expect(current_path).to eq edit_skill_compare_path(@compare.skill, @compare)
    end
  end
end
...
RSpec.describe '削除機能', type: :system do
  ...
  context '削除する' do
    it '削除できることを確認' do
      ...
      visit skill_path(@compare.skill)
      # 「削除する」ボタンがあることを確認する
      expect(page).to have_link '削除', href: skill_path(@compare.skill)
      # 削除する
      expect { find_link('削除', href: skill_path(@compare.skill)).click }.to change { Skill.count }.by(-1).and change { Compare.count }.by(-1)
      # トップページに遷移することを確認
      expect(current_path).to eq root_path
      # トップページには投稿1の内容が存在しないことを確認する（動画）
      expect(page).to have_no_link href: skill_path(@compare.skill)
    end
  end
